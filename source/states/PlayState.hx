...
import grilledlua.*;
#else
import grilledlua.LuaUtils;
import grilledlua.HScript;
#end
...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Come on you got this!', 0.2], //From 0% to 19%
		['Really...', 0.4], //From 20% to 39%
	...
	public var instancetempclude:Array<String> = [];
	#end
	...
	private var luaDebugGroup:FlxTypedGroup<grilledlua.DebugLuaText>;
	#end
	...
	override public function create()
	{
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		luaDebugGroup = new FlxTypedGroup<grilledlua.DebugLuaText>();
	...
	public function addTextToDebug(text:String, color:FlxColor) {
		var newText:grilledlua.DebugLuaText = luaDebugGroup.recycle(grilledlua.DebugLuaText);
		newText.text = text;
		newText.color = color;
		...
		luaDebugGroup.forEachAlive(function(spr:grilledlua.DebugLuaText) {
	...
	public dynamic function fullComboFunction()
	{
		...
		var ughs:Int = ratingsData[3].hits;

		ratingFC = "";
		if(songMisses == 0)
		{
			if (bads > 0 || ughs > 0) ratingFC = 'FC';
	...
	override public function update(elapsed:Float)
	{
		...
		FlxG.watch.addQuick("sec crap", curSection);
		FlxG.watch.addQuick("beat crap", curBeat);
		FlxG.watch.addQuick("step crap", curStep);
	...
	public var totalNoteCrap:Float = 0.0;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		totalNoteCrap += daRating.ratingMod;
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		{
			#if debug
			//Prevents crash specifically on debug without needing to try catch crap
	...
	public function setOnHScript(variable:String, arg:Dynamic, exclusions:Array<String> = null) {
		...
		for (script in hscriptArray) {
			...
			if(!instancetempclude.contains(variable))
				instancetempclude.push(variable);
			script.set(variable, arg);
		}
		#end
	}
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if(ret != LuaUtils.Function_Stop)
		{
			...
			{
				// Rating Percent
				ratingPercent = Math.min(1, Math.max(0, totalNoteCrap / totalPlayed));
				//trace((totalNoteCrap / totalPlayed) + ', Total: ' + totalPlayed + ', notes hit: ' + totalNoteCrap);
