...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		curWacky = FlxG.random.getObject(getIntroTextCrap());
	...
	function startIntro()
	{
		...
		credTextCrap = new Alphabet(0, 0, "", true);
		credTextCrap.screenCenter();

		// credTextCrap.alignment = CENTER;

		credTextCrap.visible = false;
		...
		// credGroup.add(credTextCrap);
	}

	function getIntroTextCrap():Array<Array<String>>
	{
		#if MODS_ALLOWED
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 15:
					addMoreText('Night');
				case 16:
					addMoreText('Funkin'); // credTextCrap.text += '\nFunkin';
		...
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					default: //Go back to normal ugly alternate boring GF
						remove(ngSpr);
						remove(credGroup);
