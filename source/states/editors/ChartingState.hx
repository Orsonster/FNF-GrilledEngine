...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		{
			PlayState.SONG = Song.parseJSONcrap(FlxG.save.data.autosave);
	...
	{
		while (crapUI.members.length > 0)
		{
			crapUI.remove(crapUI.members[0], true);
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			//AWW YOU MADE IT tempY <3333 THX SHADMAR
		...
		var playedSound:Array<Bool> = [false, false, false, false]; //Prevents ouchy GF temp sounds
		curRenderedNotes.forEachAlive(function(note:Note) {
			note.alpha = 1;
			if(curSelectedNote != null) {
	...
	function resetSection(songBeginning:Bool = false):Void
	{
		...
		FlxG.sound.music.pause();
		// Basically old crap from changeSection???
	...
	{
		// crapty null fix, i fucking hate it when this happens
		//make it look tempier if possible
		try {
