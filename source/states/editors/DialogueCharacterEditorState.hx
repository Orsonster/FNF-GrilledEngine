...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	function addAnimationsUI() {
		...
		animationDropDown = new FlxUIDropDownMenu(10, 30, FlxUIDropDownMenu.makeStrIdLabelArray([''], true), function(animation:String) {
			...
			if(character.dialogueAnimations.exists(anim)) {
				...
				var animCrap:DialogueAnimArray = character.dialogueAnimations.get(curSelectedAnim);
				offsetLoopText.text = 'Loop: ' + animCrap.loop_offsets;
				offsetIdleText.text = 'Idle: ' + animCrap.idle_offsets;

				animationInputText.text = animCrap.anim;
				loopInputText.text = animCrap.loop_name;
				idleInputText.text = animCrap.idle_name;
			}
		});
	...
	function reloadCharacter() {
		...
		if(character.jsonFile.animations.length > 0) {
			...
			var animCrap:DialogueAnimArray = character.dialogueAnimations.get(curSelectedAnim);
			ghostLoop.playAnim(animCrap.anim);
			ghostIdle.playAnim(animCrap.anim, true);
			offsetLoopText.text = 'Loop: ' + animCrap.loop_offsets;
			offsetIdleText.text = 'Idle: ' + animCrap.idle_offsets;
		}
			...
			if(UI_mainbox.selected_tab_id == 'Animations' && curSelectedAnim != null && character.dialogueAnimations.exists(curSelectedAnim)) {
				...
				for (i in 0...controlArrayLoop.length) {
					if(controlArrayLoop[i]) {
						if(i % 2 == 1) {
							animCrap.loop_offsets[1] += offsetAdd * negaMult[i];
						} else {
							animCrap.loop_offsets[0] += offsetAdd * negaMult[i];
						}
						moved = true;
					}
				}
				for (i in 0...controlArrayIdle.length) {
					if(controlArrayIdle[i]) {
						if(i % 2 == 1) {
							animCrap.idle_offsets[1] += offsetAdd * negaMult[i];
						} else {
							animCrap.idle_offsets[0] += offsetAdd * negaMult[i];
						}
						moved = true;
					}
				}
				...
				if(moved) {
					offsetLoopText.text = 'Loop: ' + animCrap.loop_offsets;
					offsetIdleText.text = 'Idle: ' + animCrap.idle_offsets;
					ghostLoop.offset.set(animCrap.loop_offsets[0], animCrap.loop_offsets[1]);
					ghostIdle.offset.set(animCrap.idle_offsets[0], animCrap.idle_offsets[1]);
				}
			}
