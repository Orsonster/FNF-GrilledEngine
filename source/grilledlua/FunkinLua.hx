...
import grilledlua.LuaUtils.LuaTweenOptions;
#if SScript
import grilledlua.HScript;
#end
import grilledlua.DebugLuaText;
import grilledlua.ModchartSprite;
...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		//alternate stuff for epic coders like me B)  *image of obama giving himself a medal*
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			var penitempam:Dynamic = LuaUtils.tweenPrepare(tag, vars);
			if(penitempam != null) {
				var curColor:FlxColor = penitempam.color;
				curColor.alphaFloat = penitempam.alpha;
				game.modchartTweens.set(tag, FlxTween.color(penitempam, duration, curColor, CoolUtil.colorFromString(targetColor), {ease: LuaUtils.getTweenEaseByString(ease),
					onComplete: function(twn:FlxTween) {
		...
		//stupid bietch alternate functions
		...
		Lua_helper.add_callback(lua, "getMoutemp", function(camera:String) {
		...
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0, updateHitbox:Bool = true) {
			if(game.getLuaObject(obj)!=null) {
				var crap:FlxSprite = game.getLuaObject(obj);
				 crap.setGraphicSize(x, y);
				if(updateHitbox) crap.updateHitbox();
				return;
			}
		...
		Lua_helper.add_callback(lua, "startDialogue", function(dialogueFile:String, music:String = null) {
			...
			#end
			{
				var crap:DialogueFile = DialogueBoxPsych.parseDialogue(path);
				if( crap.dialogue.length > 0) {
					game.startDialogue( crap, music);
