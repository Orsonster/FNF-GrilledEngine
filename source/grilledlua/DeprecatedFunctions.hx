...
class DeprecatedFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		// DEPRECATED, DONT MESS WITH THESE CRAPS, ITS JUST THERE FOR BACKWARD COMPATIBILITY
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByPrefix", function(tag:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			...
			if(PlayState.instance.modchartSprites.exists(tag)) {
				var temp:ModchartSprite = PlayState.instance.modchartSprites.get(tag);
				temp.animation.addByPrefix(name, prefix, framerate, loop);
				if(temp.animation.curAnim == null) {
					temp.animation.play(name, true);
				}
			}
		});
		...
		Lua_helper.add_callback(lua, "scaleLuaSprite", function(tag:String, x:Float, y:Float) {
			...
			if(PlayState.instance.modchartSprites.exists(tag)) {
				var crap:ModchartSprite = PlayState.instance.modchartSprites.get(tag);
				 crap.scale.set(x, y);
				 crap.updateHitbox();
				return true;
			}
